...
// Request to catch the product with the id 7
$query = $pdo->prepare("SELECT * FROM product WHERE id = :id");
$query->execute(['id' => 7]);
$productData = $query->fetch(PDO::FETCH_ASSOC);

if ($productData) {
    // Creation of a new instance of Product
    $product = new Product(
        $productData['id'],
        $productData['name'],
        json_decode($productData['photos'], true), // Decode the photos if they are stored in JSON
        $productData['price'],
    ...
    // Display the product information to check if it's correct
    var_dump($product);
