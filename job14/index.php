...
class Clothing extends AbstractProduct implements StockableInterface {
    ...
    public function addStocks(int $stock): self {
        if ($stock < 0) {
            throw new InvalidArgumentException("Stock to add cannot be negative");
        }
        $this->quantity += $stock;
        return $this;
    }
    ...
    public function removeStocks(int $stock): self {
        if ($stock < 0) {
            throw new InvalidArgumentException("Stock to remove cannot be negative");
        }
        if ($this->quantity < $stock) {
            throw new InvalidArgumentException("Not enough stock to remove");
        }
        $this->quantity -= $stock;
        return $this;
    }
